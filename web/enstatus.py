...
import requests
import re
def tvieenstatus(hosts,kind):

    s = requests.session()
    data = {'username':'admin','password':'tvie_rocks','update':'登录'}
    #post 换成登录的地址，
    res=s.post('http://%s/mgmt/login'%hosts,data)
    ...
    a = s.post("http://%s/mgmt/dwr/call/plaincall/sysInfoService.refreshSysInfo.dwr"%hosts,data2)
