...
class MyUser(AbstractBaseUser):
    ...
    #username = models.ForeignKey(settings.AUTH_USER_MODEL,unique=True,)
    ...
    def has_perms(self, perm_list, obj=None):
        for perm in perm_list:
            if not self.has_perms(perm, obj):
                return False
        return True
...
class Configuration(models.Model):
    ...
    nic = models.ManyToManyField('NICConfig', verbose_name=u'网卡列表',blank=True )
...
class NICConfig(models.Model):
    ...
    gateway = models.IPAddressField(u'网关')
...
class Server(models.Model):
    ...
    os_version  = models.CharField(u'操作系统版本',max_length=64, blank=True,null=True)
    ...
    update_at = models.DateTimeField(blank=True,auto_now_add=True,null=True)
    class Meta:
        verbose_name = '服务器'
...
class BusinessUnit(models.Model):
    ...
    contact = models.ForeignKey('UserProfile',default=None,null=True,blank=True)
...
class UserProfile(models.Model):
    user1 = models.OneToOneField('MyUser')
...
###################encoder part#############################
class EencoderStatus(models.Model):
        encoder_name = models.CharField(max_length=100, default='server',unique=True)
...
#################channels#####################################
class tvie_channels(models.Model):
    channels = models.CharField(max_length=20)
    stream = models.URLField()
    server_group = models.CharField(max_length=10,default='none')
    def __unicode__(self):
...
#################用于显示IP地址查询的历史查询记录###
class ippostionhistory(models.Model):
...
class testtest(models.Model):
    a = models.CharField(u'标题',max_length=20)
